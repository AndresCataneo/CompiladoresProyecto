...
buildDfa _ [] knownStates transitions = (knownStates, transitions) -- Caso base: worklist vacía
buildDfa afn (currentSet:worklist) knownStates transitions =
    let
        ...
        -- Procesa un solo carácter del alfabeto y actualiza los acumuladores.
            ...
            in
                ...
                then (accTrans, accStates, accWorklist)
                    ...
                    case lookup nextSet accStates of
                        -- CASO 1: Ya lo hemos visto
                        Just nextId ->
                            let newT = (currentId, char, nextId)
                            in (newT : accTrans, accStates, accWorklist)
                        
                        -- CASO 2: Es un estado nuevo
                        Nothing ->
                            let
                                newId = length accStates
        ...
        --  Importante: eliminamos duplicados en la worklist
        nuevosPendientes = filter (`notElem` (map fst newStates)) newWorklistItems

    in
        -- Llamada recursiva solo con conjuntos nuevos
        buildDfa afn (worklist ++ nuevosPendientes) newStates (transitions ++ newTransitions)
